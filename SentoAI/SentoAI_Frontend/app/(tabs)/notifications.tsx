...
export default function NotificationsScreen() {
  ...
  const params = useLocalSearchParams(); // Aici prindem autoRead: 'true' de la asistent
  ...
  // --- LOGICA DE CITIRE AUTOMATĂ ---
  useEffect(() => {
    if (params.autoRead === 'true') {
      const deCitit = `Maria, ai notificări noi. 
        Prima este: ${notifications[0].title}. ${notifications[0].desc}. 
        A doua este: ${notifications[1].title}. ${notifications[1].desc}.`;

      Speech.speak(deCitit, {
        language: 'ro-RO',
        rate: 0.85, 
      });
    }

    // Curățare: dacă Maria iese de pe ecran, asistentul tace
    return () => { Speech.stop(); };
  }, [params.autoRead]);
          ...
          <View key={item.id} style={styles.notificationCard}>
            <View style={styles.cardTop}>
              <View style={[styles.iconContainer, 
                ...
                item.type === 'success' ? styles.iconSuccess : styles.iconInfo
              ]}>
                <Ionicons 
                  ...
                  color={
                    ...
                    item.type === 'success' ? '#22C55E' : '#2D7482'
                  } 
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  notificationCard: { backgroundColor: '#fff', borderRadius: 24, padding: 20, marginBottom: 16, elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10 },
