...
export default function AssistantScreen() {
  ...
  const [statusText, setStatusText] = useState("Apasă 'START' și spune ce dorești");
  ...
  const aiSpeak = (text: string) => {
    Speech.speak(text, { language: 'ro-RO', rate: 0.85, onStart: () => setStatusText(text) });
  };
  ...
  const uploadAudio = async (uri: string) => {
    try {
      ...
      const response = await fetch('http://172.20.10.6:8000/process-audio', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.infoBox}>
          <Ionicons name="chatbubble-ellipses" size={24} color="#2D7482" style={{marginBottom: 10}} />
        ...
        >
          <Ionicons name={isListening ? "stop" : "mic"} size={70} color={isListening ? "#fff" : "#2D7482"} />
        </TouchableOpacity>
        ...
        <Text style={styles.hint}>Ține apăsat pe cerc sau folosește butoanele</Text>
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 32, fontWeight: 'bold', marginBottom: 20, color: '#1A202C' },
  infoBox: { 
  ...
  infoText: { fontSize: 19, textAlign: 'center', color: '#2C5282', fontWeight: '700', lineHeight: 26 },
  micCircle: { 
